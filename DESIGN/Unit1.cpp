...
typedef enum
{
	...
	TOSYM,
	DOWNTOSYM,
	RETURNSYM, // 共5个。ELSE，FOR，TO，DOWNTO，RETURN
	PLUSBECOMES,
	MINUSBECOMES,
	PLUSPLUS,
	MINUSMINUS // +=, -=, ++, --
...
char *SYMOUT[] = {
	...
	"CONSTSYM", "VARSYM", "PROCSYM", "PROGSYM", "ELSESYM", "FORSYM", "TOSYM", "DOWNTOSYM", "RETURNSYM", // 共5个。ELSE，FOR，TO，DOWNTO，RETURN
...
void FACTOR(SYMSET FSYS, int LEV, int &TX)
{
	...
	while (SymIn(SYM, FACBEGSYS))
	{
		...
		{
			break;
		} // 因子分析的过程中，将ELSESYM也当成其开始因子，没有对其操作会进入无限循环
		else if (SYM == TOSYM)
		{
			break;
		} // 分析为To跳出
